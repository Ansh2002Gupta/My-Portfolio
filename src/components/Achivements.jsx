...
const Achivements = () => {
    ...
    <div ref={ref} className="bg-gradient-to-b from-[#000814] to-[#003566] h-screen">
        <div className="flex flex-col items-center justify-center h-[15vh] sticky top-0 left-0 right-0">
            <h1 className="text-7xl font-bold text-[#003566]">Achivements</h1>
        </div>
        <div className="h-[85vh] flex flex-row justify-center items-center">
            <motion.div variants={trophyAnimation} initial="initial" animate={isInView && "animate"} className="relative left-0 mr-2">
                ...
                <svg width="650px" height="650px" version="1.0" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="-6.4 -6.4 76.80 76.80" enableBackground="new 0 0 64 64" xmlSpace="preserve" fill="none" stroke="#FFC300">
            ...
            <motion.div className="absolute left-60 z-40 w-[60vw] h-[80vh] grid grid-cols-3 gap-2 p-10">
                {
                    items.map((item) => (
                        <motion.div initial={{scale: "0%"}} animate={isInView && {scale: "100%", transition:{delay: item.id, duration: 0.5}}} key={item.id} className="shadow-md rounded-lg shadow-black text-center overflow-hidden px-2 py-[20%]" style={{backgroundColor: (item.id*10)%2 ? "#000814" : "#001D3D", border: item.id%2 ? "2px solid rgb(0,8,20)" : "2px solid rgb(0,29,61)", borderStyle: "inset"}} dangerouslySetInnerHTML={{ __html: item.description }}>
                        </motion.div>
                    ))           
                }
            </motion.div>
