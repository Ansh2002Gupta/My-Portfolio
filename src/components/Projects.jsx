...
const Single = ({ item }) => {
  ...
  return (
    <section ref={ref} className="flex flex-col justify-center items-center">
      <motion.div variants={animation} initial="initial" animate={isInView && "animate"} className="mt-10    p-3 bg-[#000814] flex flex-col md:flex-row max-h-[80vh] max-w-[100vw] md:max-w-[65vw] h-auto rounded-lg shadow-lg shadow-black gap-2 md:gap-0">
        <motion.img variants={animation} className="p-1 mr-7 w-full h-[200px] md:w-[40%] md:h-[100%] border-[#FFD60A] border-2" src={item.image} alt="default_image" />
        <motion.div variants={animation} className="flex flex-col mx-2 gap-4 md:gap-2">
          <motion.h2 variants={animation} className="text-xl sm:text-2xl md:text-4xl text-[#FFC300]">
            {item.title}
          </motion.h2>
          <motion.div variants={animation} className="text-xs sm:text-sm md:text-base min-w-[25%] min-h-[25%] overflow-hidden">
            {item.description}
          </motion.div>
          <motion.div variants={animation} className="flex flex-row gap-4">
            {item.technology.map((tech, id) => (
              <span className="p-1 md:p-2 text-sm md:text-base md:font-bold ring-2 ring-[#FFC300] font-['Battambang'] rounded-lg" key={id}>
                {tech}
              </span>
            ))}
          </motion.div>
          <motion.a variants={animation} className="text-center px-2 py-2 md:py-3 rounded-md bg-[#FFC300] font-['Tajawal'] text-[#000814] text-bold hover:bg-[#FFD60A]" href={item.link}>
            Live Demo
          </motion.a>
        </motion.div>
      </motion.div>
  ...
  return (
    <div ref={ref} className="bg-gradient-to-b from-[#000814] to-[#003566]">
      <div className="flex flex-col items-center justify-center h-[15vh] sticky top-0 left-0 right-0">
        <h1 className="text-5xl md:text-7xl font-bold text-[#003566]">My Projects</h1>
